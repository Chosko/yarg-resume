 ...
 *
*/
Handlebars.registerHelper('include', function(options, param) {
  file = options + ".hbs";
  filePathed = path.resolve(__dirname + '/includes', file);
  if (filePathed) {
    include = fs.readFileSync(filePathed, 'utf8');
    var fn = Handlebars.compile(include)({
      resume: param.data.root.resume
    });
    return new Handlebars.SafeString(fn);
  }
});
...
function render(resume, tpl, css) {
    var css = fs.readFileSync(path.resolve(__dirname, css), 'utf-8'),
        template = fs.readFileSync(path.resolve(__dirname, tpl), 'utf-8'),
